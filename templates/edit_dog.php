    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
      if(isset($_POST['action'])){
       updateDog($_POST['preferred_environment'], $_POST['dog_breed'], $_POST['size'],
                       ...
                       $_POST['price'], $_POST['ok_with_kids'], $_POST['ok_with_other_pets'],
...
<form name="mainForm" action="<?php $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
 ...
 <div class="form-group">
  Size: <br>
  <input type="text"  name="size" class="form-control" name="color"   value="<?php echo $item['dog_size']?>" />
  </div> 
  <div class="form-group">
    Color:
...
<div class="form-group">
 ...
 <div class="form-group">
  ...
  <div class="form-group">
    Price:
    <input type="number" class="form-control" name="price"   max="10000" min="0"  value="<?php echo $item['price']?>" />
  </div>
  <div class="form-group">
...
<p><a href="javascript:addElement();" ><label type="button" class="btn btn-outline-primary btn-sm">Add condition</label></a></p>
 ...
 <div>
    ...
    <!-- <button class='btn btn-outline-primary btn-sm del_btn btn-outline-danger' onclick="deleteItem(this); '<?php// deleteUnderlyingCondition($dog_id, $hc['underlying_conditions'])?>'" >Remove</button>  -->
        ...
        <button name="deletebutton" style="color:red" class="login100-form-btn" >Delete Conditions</button> <br>
  </div>
<?php endforeach; ?>
</div>
  <div class="form-group">
...
<?php
function updateDog($preferred_environment, $dog_breed, $dog_size, $color, $activeness_level, $age, $name, $current_location, $shots_uptodate, $gender, $hypoallergenic, $price, $ok_with_kids, $ok_with_other_pets, $description, $dog_id) 
{
        global $db;
        $query = "UPDATE dog SET preferred_environment=:preferred_environment, dog_breed=:dog_breed, dog_size=:dog_size, color=:color, activeness_level=:activeness_level, age=:age, name=:name, current_location=:current_location, shots_uptodate=:shots_uptodate, gender=:gender, hypoallergenic=:hypoallergenic, price=:price, ok_with_kids=:ok_with_kids, ok_with_other_pets=:ok_with_other_pets, description=:description WHERE DogID=:dog_id";
        $statement = $db->prepare($query);
